...
def _resolve_api_return_schema(cls: type) -> Any:
    ...
    resolves ``UserNotFound`` → ``ErrorBody``.
    """
...
def _is_api_return_subclass(arm: Any) -> bool:
    return isinstance(arm, type) and issubclass(arm, APIReturn)
...
def _parse_api_return_arm(arm: type, view_func: Callable) -> tuple[int, Any]:
    ...
    schema = _resolve_api_return_schema(arm)
    return code, schema
