...
2) **Schema integrates with the ORM.** You can return querysets or ORM objects directly:
...
@api.get("/tasks/first")
def tasks_details(request) -> TaskSchema:
    return Task.objects.first()
```

3) **Authentication via `request` attributes**, not dependency injection — matches regular Django views. See [Authentication](guides/authentication.md) for details.
