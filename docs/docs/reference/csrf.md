...
## How to protect against CSRF with Django Hattori
...
By default, **Django Hattori** has CSRF protection turned **OFF** for all operations, but will automatically enable csrf **for Cookie based** authentication:
...
api = HattoriAPI(auth=django_auth)
```
...
- The `ensure_csrf_cookie` decorator works only on a Django `HttpResponse` (and subclasses like `JsonResponse`) and not on a dict like most Django Hattori decorators.
