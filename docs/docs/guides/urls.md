...
A reverse URL name is generated for each method in a Django Hattori Schema (or `Router`).
...
The default URL namespace is built by prepending the Schema's version with `"api-"`, however you can explicitly specify the namespace by overriding the `urls_namespace` attribute of the `HattoriAPI` Schema class.

```python

api = HattoriAPI(auth=token_auth, version='2')
