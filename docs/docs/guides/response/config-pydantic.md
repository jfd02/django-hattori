...
There are many customizations available for a **Django Hattori `Schema`**, via the schema's
[Pydantic `model_config`](https://docs.pydantic.dev/latest/api/config/). 

!!! info
    Under the hood **Django Hattori** uses [Pydantic Models](https://pydantic-docs.helpmanual.io/usage/models/)
...
Using Pydantic's example in **Django Hattori** can look something like:
