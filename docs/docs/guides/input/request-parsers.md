...
other content types (like YAML, XML, CSV) or use faster JSON parsers, **Django Hattori** provides a `parser` configuration.

```python
api = HattoriAPI(parser=MyYamlParser())
```
...
api = HattoriAPI(parser=ORJSONParser())
```
