...
!!! tip
    ...
    So, with just that type declaration, **Django Hattori** gives you automatic request "parsing" and validation.
...
!!! tip
    Notice that, since **Django Hattori** uses a default type of `str` for unannotated parameters, the value the
...
!!! note
    Notice that here we used a `Path` source hint to let **Django Hattori** know that this schema will be applied to path parameters.
...
![Django Hattori Swagger](../../img/tutorial-path-swagger.png)
