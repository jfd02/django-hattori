...
**Django Hattori** comes with an easy way to split your API into multiple modules using Routers.
...
Now let's add a few operations to `events/api.py`. The trick is that instead of using the `HattoriAPI` class, you use the **Router** class:
...
In your top level project folder (next to `urls.py`), create another `api.py` file with the main `HattoriAPI` instance:
...
**Django Hattori** makes it possible to include a router into another router as many times as you like, and finally include the top level router into the main `api` instance.
