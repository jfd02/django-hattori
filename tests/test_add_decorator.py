...
def test_router_add_decorator_operation_mode():
    ...
    @router.get("/test")
    def endpoint(request) -> MessageDecoratedResult:
        return {"message": "test"}
...
def test_router_add_decorator_view_mode():
    ...
    @router.get("/test")
    def endpoint(request) -> MessageResult:
        return {"message": "test"}
...
def test_api_add_decorator_operation_mode():
    ...
    @api.get("/test1")
    def endpoint1(request) -> MessageDecoratedResult:
        return {"message": "test1"}

    @api.get("/test2")
    def endpoint2(request) -> MessageDecoratedResult:
...
def test_multiple_decorators():
    ...
    @router.get("/test")
    def endpoint(request) -> MessageFullDecoratedResult:
        return {"message": "test"}
...
def test_decorator_cascading():
    ...
    api.add_decorator(
        lambda f: wraps(f)(
            lambda req, *a, **k: {**f(req, *a, **k), "api": True}
        )
    )
    ...
    parent_router.add_decorator(
        lambda f: wraps(f)(
            lambda req, *a, **k: {**f(req, *a, **k), "parent": True}
        )
    )
    ...
    child_router.add_decorator(
        lambda f: wraps(f)(
            lambda req, *a, **k: {**f(req, *a, **k), "child": True}
        )
    )

    @child_router.get("/test")
    def endpoint(request) -> MessageCascadeResult:
        return {"message": "test"}
...
def test_decorator_with_path_params():
    ...
    def endpoint(request, item_id: int) -> ItemIdDecoratedResult:
