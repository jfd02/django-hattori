...
from hattori import Form, Query, Router, Schema
...
class SomeOtherResponse(Schema):
    some: SomeModel
    other: OtherModel
...
@router.post("/test1")
def view1(request, some: SomeModel) -> SomeModel:
    assert isinstance(some, SomeModel)
    return some


@router.post("/test2")
def view2(request, some: SomeModel, other: OtherModel) -> SomeOtherResponse:
    assert isinstance(some, SomeModel)
...
def view4(request, form: OtherModel = Form(...)) -> OtherModel:
    assert isinstance(form, OtherModel)
    return form


@router.post("/test_query")
def view4query(request, q: OtherModel = Query(...)) -> OtherModel:
    assert isinstance(q, OtherModel)
    return q


@router.post("/selfref")
def view5(request, obj: SelfReference) -> SelfReference:
    assert isinstance(obj, SelfReference)
    return obj


@router.post("/model-default")
def view6(request, obj: OtherModel = None) -> OtherModel | None:
