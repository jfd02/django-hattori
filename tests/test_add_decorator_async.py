...
async def test_router_add_decorator_async_operation_mode():
    ...
    @router.get("/test")
    async def endpoint(request) -> MessageAsyncDecoratedResult:
...
async def test_router_add_decorator_async_view_mode():
    ...
    @router.get("/test")
    async def endpoint(request) -> MessageResult:
...
async def test_api_add_decorator_async():
    ...
    @api.get("/test1")
    async def endpoint1(request) -> MessageAsyncDecoratedResult:
        await asyncio.sleep(0)
        return {"message": "test1"}

    @api.get("/test2")
    async def endpoint2(request) -> MessageAsyncDecoratedResult:
        await asyncio.sleep(0)
...
async def test_mixed_sync_async_decorators():
    ...
    @router.get("/async")
    async def async_endpoint(request) -> TypeSyncDecoratedResult:
        await asyncio.sleep(0)
        return {"type": "async"}

    @router.get("/sync")
    def sync_endpoint(request) -> TypeSyncDecoratedResult:
        return {"type": "sync"}
...
async def test_mixed_sync_async_endpoints_same_router():
    ...
    async def async_endpoint(
        request,
    ) -> EndpointUniversalDecoratedResult:
        await asyncio.sleep(0)
