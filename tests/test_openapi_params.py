...
def operation_1(request) -> DocstringsResult:
    """
...
def operation5(request) -> OpenApiExtraResult:
    return {"openapi_extra": True}


@api.get("/not-included", include_in_schema=False)
def not_included(request) -> bool:
    return True
