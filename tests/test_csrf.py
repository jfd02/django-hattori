...
csrf_ON = HattoriAPI(urls_namespace="csrf_ON", auth=AnyCookieAuth())  # , csrf=True
csrf_ON_with_django_auth = HattoriAPI(urls_namespace="csrf_ON_django", auth=django_auth)


@csrf_OFF.post("/post")
def post_off(request) -> SuccessResult:
    return {"success": True}


@csrf_ON.post("/post")
def post_on(request) -> SuccessResult:
    return {"success": True}


@csrf_ON.post("/post/csrf_exempt")
@csrf_exempt
def post_on_with_exempt(request) -> SuccessResult:
    return {"success": True}
...
@ensure_csrf_cookie
def obtain_csrf_token_get(request) -> SuccessResult:
...
@ensure_csrf_cookie
@csrf_exempt
def obtain_csrf_token_post(request) -> SuccessResult:
    return JsonResponse(data={"success": True})


@csrf_ON_with_django_auth.get("/obtain_csrf_token_get", auth=None)
@ensure_csrf_cookie
def obtain_csrf_token_get_no_auth_route(request) -> SuccessResult:
    return JsonResponse(data={"success": True})
...
@ensure_csrf_cookie
@csrf_exempt
def obtain_csrf_token_post_no_auth_route(request) -> SuccessResult:
    return JsonResponse(data={"success": True})
...
def test_csrf_cookie_auth():
    ...
    api = HattoriAPI(auth=cookie_auth)

    @api.post("/test")
    def test_view(request) -> SuccessResult:
        return {"success": True}
...
def test_docs():
    ...
    api = HattoriAPI(auth=AnyCookieAuth())
...
def test_no_auth_csrf_exempt():
    ...
    api = HattoriAPI(urls_namespace="test_no_auth_csrf")

    @api.post("/create")
    def create_item(request) -> StatusResult:
